...
**Класс Basket** отвечает за отображение и управление корзиной товаров. Он наследуется от класса Component и реализует интерфейс IBasket. Этот класс отображает список товаров в корзине, общую сумму заказа и кнопку для перехода к оформлению заказа
  **поля:**
  _basketItems: HTMLElement — контейнер для отображения списка товаров в корзине.
  _totalPrice: HTMLElement — элемент для отображения общей суммы заказа.
  _button: HTMLButtonElement — кнопка для перехода к оформлению заказа.
  **методы:**
  constructor(container: HTMLElement, events: EventEmitter) rонструктор инициализирует корзину. Он находит элементы DOM для списка товаров, общей суммы и кнопки. Также настраивает обработчик события для кнопки, который генерирует событие order:open при клике.
  set basketItems(items: HTMLElement[]) - устанавливает список товаров в корзине. Если корзина не пуста, отображает товары и активирует кнопку оформления заказа. Если корзина пуста, отображает сообщение "Корзина пуста" и деактивирует кнопку.
  set totalPrice(totalPrice: number)- устанавливает общую сумму заказа.

**Класс ProductBasket** отвечает за отображение отдельного товара в корзине. Он наследуется от класса Component и реализует интерфейс IProductBasket. Этот класс отображает название товара, его порядковый номер, цену и кнопку удаления
  **поля:**
  _index: HTMLElement — элемент для отображения порядкового номера товара.
  ...
  **методы:**
  constructor(container: HTMLElement, actions?: IActionsBasket) - конструктор инициализирует карточку товара в корзине. Он находит элементы DOM для названия, порядкового номера, цены и кнопки удаления. Также настраивает обработчик события для кнопки удаления, который удаляет товар из DOM и вызывает переданный обработчик onClick.
  set title(value: string) - устанавливает название товара.
  set index(value: number) - устанавливает порядковый номер товара (увеличивает значение на 1 для отображения).
...
**Класс Modal** управляет модальными окнами. Он отвечает за открытие, закрытие и отображение содержимого модального окна
  ...
  **методы:**
   constructor(container: HTMLElement, protected events: IEvents) - конструктор инициализирует модальное окно. Он находит элементы DOM для кнопки закрытия и контейнера содержимого. Также настраивает обработчики событий.
   set content(value: HTMLElement) - устанавливает содержимое модального окна. Очищает текущее содержимое и заменяет его на переданное значение.

  open() - открывает модальное окно, добавляя класс modal_active к контейнеру. Генерирует событие modal:open, чтобы уведомить другие компоненты об открытии модального окна.

  close() - закрывает модальное окно, удаляя класс modal_active из контейнера. Очищает содержимое модального окна и генерирует событие modal:close, чтобы уведомить другие компоненты о закрытии модального окна.

  render(data: IModalData): HTMLElement - устанавливает содержимое модального окна и открывает его. Возвращает контейнер модального окна.
